...
export interface VehicleDatabaseEntry {
  make: string;
  model: string;
  basePriceRange: [number, number]; // min, max in thousands
  typicalSegment: 'luxury' | 'performance' | 'suv' | 'sedan' | 'compact' | 'ev' | 'crossover' | 'convertible';
 ...
 */
export function getRandomVehicleForSegment(
  segment: 'luxury' | 'performance' | 'suv' | 'sedan' | 'compact' | 'ev' | 'crossover' | 'convertible',
  rng: { nextFloat: () => number; pick: <T>(arr: T[]) => T }
): Omit<VehicleDatabaseEntry, 'years'> & { year: number } {
 ...
 */
export function getYearPriceMultiplier(year: number): number {
  ...
  if (age === 1) return 0.92; // 1 year old (8% depreciation)
  if (age === 2) return 0.85; // 2 years old (15% depreciation)
  if (age === 3) return 0.78; // 3 years old (22% depreciation)
  if (age === 4) return 0.72; // 4 years old (28% depreciation)
  if (age === 5) return 0.66; // 5 years old (34% depreciation)
