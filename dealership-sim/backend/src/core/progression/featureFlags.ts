 ...
 */
export function getMaxAdvisors(state: GameState): number {
  let max = BASE_CAPABILITIES.maxAdvisors; // Start with 1
  
  // Check each upgrade that increases advisor count
  if (state.purchasedUpgrades.includes('second_advisor')) max = Math.max(max, 2);
  if (state.purchasedUpgrades.includes('third_advisor')) max = Math.max(max, 3);
  if (state.purchasedUpgrades.includes('fourth_advisor')) max = Math.max(max, 4);
  if (state.purchasedUpgrades.includes('fifth_advisor')) max = Math.max(max, 5);
  if (state.purchasedUpgrades.includes('sixth_advisor')) max = Math.max(max, 6);
  if (state.purchasedUpgrades.includes('elite_team')) max = Math.max(max, 8);
 ...
 */
export function getMaxTechnicians(state: GameState): number {
  let max = BASE_CAPABILITIES.maxTechnicians; // Start with 0 (no service)
  
  if (state.purchasedUpgrades.includes('service_department')) max = Math.max(max, 2);
  if (state.purchasedUpgrades.includes('additional_techs_1')) max = Math.max(max, 4);
  if (state.purchasedUpgrades.includes('additional_techs_2')) max = Math.max(max, 6);
  if (state.purchasedUpgrades.includes('service_empire')) max = Math.max(max, 10);
 ...
 */
export function getMaxInventorySlots(state: GameState): number {
 ...
 */
export function hasServiceAccess(state: GameState): boolean {
 ...
 */
export function canAutoAdvance(state: GameState): boolean {
 ...
 */
export function canAutoPricer(state: GameState): boolean {
 ...
 */
export function canOptimizeMarketing(state: GameState): boolean {
 ...
 */
export function getMaxSpeed(state: GameState): 1 | 5 | 30 {
  if (state.purchasedUpgrades.includes('speed_boost_30x')) return 30;
  if (state.purchasedUpgrades.includes('speed_boost_5x')) return 5;
  return 1;
}
 ...
 */
export function canBuyBulk(state: GameState): boolean {
 ...
 */
export function hasIncreasedMarketing(state: GameState): boolean {
 ...
 */
export function hasPremiumAccess(state: GameState): boolean {
 ...
 */
export function getMaxMarketingSpend(state: GameState): number {
 ...
 */
export function isFeatureUnlocked(state: GameState, feature: string): boolean {
  switch (feature) {
    case 'service':
 ...
 */
export function validateInventoryPurchase(state: GameState, quantity: number): {
  valid: boolean;
  error?: string;
  maxAllowed?: number;
} {
 ...
 */
export function validateHiring(state: GameState, role: 'advisor' | 'technician'): {
  valid: boolean;
  error?: string;
} {
  if (role === 'advisor') {
