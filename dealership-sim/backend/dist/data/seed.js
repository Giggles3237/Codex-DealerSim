...
const createTechnicians = (rng, maxTechnicians = 0) => {
    // Start with NO technicians - service is locked initially
    return [];
};
// Create minimal starter inventory for incremental game feel
const createStarterInventory = (rng, coefficients, pricingState, count = 4, maxSlots = 15) => {
    const vehicles = [];
    const starterSegments = ['compact', 'sedan']; // Only cheap segments to start
...
const createInventory = (rng, coefficients, pricingState, maxSlots = 15) => {
    const segments = ['suv', 'sedan', 'crossover', 'compact', 'ev', 'performance', 'luxury', 'convertible'];
    const vehicles = [];
    // Calculate avg cost based on lot size (smaller lot = cheaper vehicles)
    const avgCostPerUnit = 15000 + (maxSlots * 150);
